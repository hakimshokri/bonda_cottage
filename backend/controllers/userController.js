...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        const mailOptions = {
            ...
            to: email,
            subject: "Welcome to Bonda Cottage",
            html: `<div style="font-family: Arial, sans-serif; background-color: #f5f5f5; padding: 20px; text-align: center;">
                        <h1 style="color: #333;">Welcome to Bonda Cottage</h1>
                        <p style="color: #666;">Hello ${name},</p>
                        <p style="color: #666;">Thank you for registering with Bonda Cottage. Your account has been successfully created.</p>
                        <p style="color: #666;">Best regards,<br>Bonda Cottage</p>
                    </div>`,
        }
...
// API to check cottage slot availability
const checkSlotAvailability = async (req, res) => {
    ...
    try {
        ...
        const cottageData = await cottageModel.findById(cottageId).select('-password')

        if (!cottageData.status === "Available") {
            return res.json({ success: false, message: "Cottage not available" })
        }
...
// API to book cottage
const bookCottage = async (req, res) => {

    try {

        const { userId, cottageId, startDate, endDate, fullName, email, phone, guests, specialRequest, amount } = req.body
        ...
        delete cottageData.slots_booked
        ...
        // save new slots data in cottageData
...
export { registerUser, loginUser, logout, sendVerifyOtp, verifyEmail, verifyResetOtp, isAuthenticated, sendResetOtp, resetPassword, getProfile, updateProfile, checkSlotAvailability, bookCottage, listBooking, cancelBooking, paymentStripe, verifyStripe }